...
def receptive_field(model, input_size, batch_size=-1, device='cuda'):
    ...
    def register_hook(module):
        ...
        def hook(module, input, output):
            ...
            receptive_field[m_key]['layer_cls'] = class_name
            ...
            if not receptive_field['0']['conv_stage']:
                ...
                if class_name == 'Conv2d' or class_name == 'MaxPool2d' or class_name == 'AvgPool2d' or class_name == 'Conv3d' or class_name == 'MaxPool3d':
                    ...
                    receptive_field[m_key]['r'] = int(p_r + ((kernel_size - 1) * dilation) * p_j)
                    receptive_field[m_key]['start'] = p_start + ((kernel_size - 1) / 2 - padding) * p_j
                elif class_name == 'BatchNorm2d' or class_name in ['ReLU', 'Tanh', 'Sigmoid'] or class_name == 'Bottleneck' or class_name == 'BatchNorm3d':
    ...
    receptive_field['0']['layer_cls'] = 'Input'
    receptive_field['0']['j'] = 1.0
    receptive_field['0']['r'] = 1
    ...
    print('------------------------------------------------------------------------------')
    line_new = '{:>23}  {:>10} {:>10} {:>10} {:>15} '.format('Layer (type)', 'map size', 'start', 'jump',
                                                             'receptive_field')
    print(line_new)
    print('==============================================================================')
    total_params = 0
    ...
    for layer in receptive_field:
        ...
        if len(receptive_field[layer]['output_shape']) == 4 or len(receptive_field[layer]['output_shape']) == 5:
            line_new = '{:7} {:15}  {:>10} {:>10} {:>10} {:>15} '.format(
                '',
                f'{layer} ({receptive_field[layer]["layer_cls"]})',
    ...
    if layer in receptive_field_dict:
        ...
        if np.any([unit_position[idx] < 0 or
            ...
            if len(unit_position) == 2:
                raise Exception(
                    'Unit position outside spatial extent of the feature tensor ((H, W) = (%d, %d)) ' % tuple(
                        feat_map_lim))
            else:
                raise Exception(
                    'Unit position outside spatial extent of the feature tensor ((D, H, W) = (%d, %d, %d)) ' % tuple(
                        feat_map_lim))
